...
- **LTX 2.3 Audio Recover** — LTX adaptation of MAINodes' H3 Audio Recover. Retimes the regenerated clip's audio back to the original clock using the same hold map as the video (phase vocoder, so pitch is kept). Wiring: audio from `VAEDecodeAudio` of the **regenerated** latent, `hold_map_used` from the same `LTX23TimeSmear` the video used (not the raw oracle `hold_map` — the smear aligns it to the image batch and snaps the tail to the legal 8k+1 grid). `fps_mode` defaults to **auto**: the samples-per-frame clock is derived from the decoded audio's actual length and the hold map, so a wrong `fps` widget or LTX's decode clock can't desync the retime; use `manual` to pin `fps` and get a drift warning instead. If pass 2's audio rows were not seeded, blend the original clip's audio back in via `reference` + `reference_mix=1` to fix rushed lipsync in held regions. The node prints a clock diagnostic per run (map frames, audio duration, effective fps) — check the console if lipsync drifts.
